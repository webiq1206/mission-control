...
export default async function ApprovalsPage() {
  ...
  const urgentCount = pending.filter(a => ['urgent','critical','high'].includes(a.urgency as string)).length
        ...
        <details open={acknowledgments.length > 0 && acknowledgments.length <= 3}>
          ...
          {acknowledgments.length === 0 ? (
            <div className="card" style={{ padding: 'var(--sp-4)', textAlign: 'center' }}>
              <div className="body-xs">
                No acknowledgments pending. These are informational items that don&apos;t require a decision — just confirmation you&apos;ve seen them.
              </div>
            </div>
          ) : (
            <>
