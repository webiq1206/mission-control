...
export async function GET(req: NextRequest) {
  ...
  const reports = await db.all('SELECT * FROM reports ORDER BY sent_at DESC LIMIT 50')
...
export async function POST(req: NextRequest) {
  ...
  const cols = [] as {name:string}[] as { name: string }[]
  ...
  await db.run(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `, id, body.title, body.recipient, body.entity_scope, '', html, emailStatus)
